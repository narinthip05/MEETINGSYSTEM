                    ...
                    <div className="modal-overlay" onClick={(e) => {
                        ...
                        >
                            ...
                            <h2 style={{ textAlign: "center" }}>รายละเอียดห้องประชุม</h2> {/* ✅ ให้หัวข้ออยู่ตรงกลาง */}
                                ...
                                <p><strong>วันที่ :</strong> {selectedRequest?.Requestdate}</p>
                                <p><strong>ชื่อห้องประชุม :</strong> {selectedRequest?.Roomname}</p>
                                <p><strong>ที่ตั้ง:</strong> {selectedRequest?.Location}</p>
                                <p><strong>ชื่อผู้ใช้/รหัสพนักงาน :</strong> {selectedRequest?.Username}</p>
                                <p><strong>ชื่อคอมพิวเตอร์:</strong> {selectedRequest?.Machinename}</p>
                                <p><strong>ประเภทเครื่อง :</strong> {selectedRequest?.Equipmenttype}</p>
                                <p><strong>รุ่น:</strong> {selectedRequest?.Model}</p>
                                <p><strong>เลขที่สัญญา :</strong> {selectedRequest?.Contractyear}</p>
                                <p><strong>ฮาร์ดดิส (Hard disk) :</strong> {selectedRequest?.Harddisk}</p>
                                <p><strong>แรม (RAM):</strong> {selectedRequest?.Ram}</p>
                                <p><strong>ปัญหา/ความต้องการ :</strong> {selectedRequest?.Problemneed}</p>
                                <p><strong>วิธีการแก้ไข :</strong> {selectedRequest?.Resolution}</p>
                            
                            <button
                                onClick={closeModal}
                                style={{ display: "block", margin: "20px auto", color: "#fff", backgroundColor: "#989696" }}>ปิด</button>
                        </div>
                    </div>
                )}

                {(modalType === "edit" || modalType === "add") && selectedRequest && (
                    <div className="modal-overlay" onClick={(e) => {
                        if ((e.target as HTMLElement).classList.contains("modal-overlay")) {
                            closeModal();
                        }
